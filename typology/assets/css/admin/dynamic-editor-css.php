...
if ( !empty( $uppercase ) ) {
  foreach ( $uppercase as $text_class => $val ) {
    if ( $val ){
      echo '.editor-styles-wrapper '. str_replace(', ',', .editor-styles-wrapper ', $text_class .'{text-transform: uppercase;}' );
    } else {
      echo '.editor-styles-wrapper '. str_replace(', ',', .editor-styles-wrapper ', $text_class .'{text-transform: none;}' );
    }
  }
}
